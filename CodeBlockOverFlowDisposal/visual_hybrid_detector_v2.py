...
class VisualHybridDetectorV2:
    ...
    def detect_coordinate_based(self, page, page_number: int) -> List[Dict]:
        ...
        for y_pos, line_chars in lines.items():
            ...
            for char in line_chars:
                # より厳密な判定（0.5pt以上の超過）
                if char['x1'] > text_right_edge + 0.5:
    ...
    def process_pdf(self, pdf_path: Path) -> List[Dict]:
        ...
        print("視覚的判断優先ハイブリッド検出 v2（目視結果反映版）")
    ...
    def save_results(self, results: List[Dict], pdf_path: Path, output_path: Path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(f"# {pdf_path.name} 視覚的判断優先ハイブリッド検出結果 v2\n\n")
            f.write("**目視結果反映版**\n\n")
            ...
            # 検出パラメータ
            f.write("## 検出パラメータ\n\n")
            f.write("- 座標ベース: コードブロック内で本文領域右端を0.5pt以上超える\n")
            f.write("- 矩形ベース: コードブロックの右端を0.5pt以上超える\n")
            ...
            # 検出漏れの分析
            if pdf_path.name in self.known_overflows:
                known_pages = self.known_overflows[pdf_path.name]
                detected_pages = set([r['page_number'] for r in results])
                missing_pages = known_pages - detected_pages
                
                if missing_pages:
                    f.write("## 検出漏れページの分析\n\n")
                    f.write(f"以下のページは目視で確認されたが、座標・矩形ベースでは検出されなかった:\n\n")
                    f.write(f"{', '.join(map(str, sorted(missing_pages)))}\n\n")
                    f.write("これらのページは視覚的判断により検出済み。\n\n")
            
            # 注記
            f.write("## 注記\n\n")
            f.write("- 視覚的判断を優先し、ユーザー確認済みのはみ出しは必ず検出\n")
            f.write("- 座標・矩形ベースの検出も並行して実施\n")
            f.write("- 複数の方法で検出された場合は信頼性が高い\n")
            f.write("- ページ105は誤検知として除外済み\n")


def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(
        description='視覚的判断優先ハイブリッド検出器 v2（目視結果反映版）'
    )
    ...
    if args.output:
        ...
        output_path = Path(f"testresult_{pdf_path.stem}_visual_v2.md")
    
    # 検出器を初期化
    detector = VisualHybridDetectorV2()
