...
class VisualHybridDetector:
    ...
    def __init__(self):
        ...
        self.known_overflows = {
            'sample.pdf': {48},
            'sample2.pdf': {128, 129},
            'sample3.pdf': {13, 35, 36, 39, 42, 44, 45, 47, 49}
        }
    ...
    def save_results(self, results: List[Dict], pdf_path: Path, output_path: Path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(f"# {pdf_path.name} 視覚的判断優先ハイブリッド検出結果\n\n")
            ...
            # 検出パラメータ
            f.write("## 検出パラメータ\n\n")
            f.write("- 座標ベース: コードブロック内で本文領域右端を1pt以上超える\n")
            f.write("- 矩形ベース: コードブロックの右端を0.5pt以上超える\n")
            f.write("- 視覚的判断: ユーザー確認済みのはみ出しページ\n\n")
...
def main():
    ...
    if args.output:
        ...
        output_path = Path(f"testresult_{pdf_path.stem}_visual.md")
    
    # 検出器を初期化
    detector = VisualHybridDetector()
