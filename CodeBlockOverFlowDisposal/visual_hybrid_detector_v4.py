...
class VisualJudgmentManager:
    ...
    def add_known_overflow(self, pdf_name: str, page_number: int):
        """既知のはみ出しページを追加"""
        if pdf_name not in self.data['known_overflows']:
            self.data['known_overflows'][pdf_name] = set()
        self.data['known_overflows'][pdf_name].add(page_number)
        
        # 誤検知から削除
        if pdf_name in self.data['false_positives']:
            self.data['false_positives'][pdf_name].discard(page_number)
    
    def add_false_positive(self, pdf_name: str, page_number: int):
        """誤検知ページを追加"""
        if pdf_name not in self.data['false_positives']:
            self.data['false_positives'][pdf_name] = set()
        self.data['false_positives'][pdf_name].add(page_number)
        
        # 既知のはみ出しから削除
        if pdf_name in self.data['known_overflows']:
            self.data['known_overflows'][pdf_name].discard(page_number)
...
class VisualHybridDetectorV4:
    ...
    def save_results(self, results: List[Dict], pdf_path: Path, output_path: Path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(f"# {pdf_path.name} 視覚的判断優先ハイブリッド検出結果 v4\n\n")
            f.write("**英数字・ASCII記号のみ判定版**\n\n")
            ...
            # 検出パラメータ
            f.write("## 検出パラメータ\n\n")
            f.write(f"- 座標ベース: コードブロック内で本文領域右端を{self.config.coordinate_threshold_pt}pt以上超える英数字・ASCII記号\n")
                ...
                for result in sorted(results, key=lambda r: r['page_number']):
                    ...
                    if result['detections']['coordinate']:
                        ...
                        for i, ov in enumerate(coord_ovs[:3], 1):
                            ...
                            if ov.get('excluded_count', 0) > 0:
                                f.write(f"   （日本語等{ov['excluded_count']}文字を除外）\n")
                        if len(coord_ovs) > 3:
                            f.write(f"... 他 {len(coord_ovs)-3} 件\n")
                        f.write("\n")
                    
                    # 視覚的判断
                    if result['detections']['visual'] and 'coordinate' not in result['detection_methods']:
                        f.write("**視覚的判断**: ユーザー目視確認済みのはみ出し\n")
                        f.write("（座標ベースでは検出されず）\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='視覚的判断優先ハイブリッド検出器 v4'
    )
    parser.add_argument('pdf_file', help='検査するPDFファイル')
    parser.add_argument('-o', '--output', help='出力ファイル名', default=None)
    parser.add_argument('--add-overflow', nargs=2, metavar=('PDF', 'PAGE'),
                       help='既知のはみ出しページを追加')
    parser.add_argument('--add-false-positive', nargs=2, metavar=('PDF', 'PAGE'),
                       help='誤検知ページを追加')
    ...
    # 検出器を初期化
    detector = VisualHybridDetectorV4()
    
    # 設定の更新
    if args.add_overflow:
        pdf_name, page = args.add_overflow
        detector.visual_manager.add_known_overflow(pdf_name, int(page))
        print(f"追加: {pdf_name} ページ{page}を既知のはみ出しに登録")
        
    if args.add_false_positive:
        pdf_name, page = args.add_false_positive
        detector.visual_manager.add_false_positive(pdf_name, int(page))
        print(f"追加: {pdf_name} ページ{page}を誤検知に登録")
    
    if args.save_config:
        detector.visual_manager.save_data()
        print("視覚的判断データを保存しました")
        return
    ...
    if args.output:
        ...
        output_path = Path(f"testresult_{pdf_path.stem}_visual_v4.md")
