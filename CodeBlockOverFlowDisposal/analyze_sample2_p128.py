#!/usr/bin/env python3
"""
sample2.pdf の128ページ（偶数ページ）の詳細分析
"""
...
def analyze_page_128():
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        page = pdf.pages[127]  # 128ページ（0-indexed）
        
        print("=== ページ128（偶数ページ）の詳細分析 ===")
        ...
        # 偶数ページ：右マージン15mm
        mm_to_pt = 72 / 25.4
        right_margin_pt = 15 * mm_to_pt
        expected_text_right = page.width - right_margin_pt
        
        print(f"\n=== 偶数ページの本文右端（理論値） ===")
        print(f"ページ幅: {page.width:.1f}pt")
        print(f"右マージン: 15mm = {right_margin_pt:.1f}pt")
        print(f"本文右端（理論値）: {expected_text_right:.1f}pt")
        
        # 48ページのrの位置（438.0pt）との比較
        print(f"\n48ページの'r'位置: 438.0pt")
        print(f"理論値との差: {438.0 - expected_text_right:.1f}pt")
