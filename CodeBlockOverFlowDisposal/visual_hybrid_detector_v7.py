...
class OverflowDetectionConfig:
    ...
    def __init__(self):
        ...
        # 矩形基準検出の閾値（v7で調整）
        self.rect_overflow_threshold = 1.0  # 矩形から1pt超えたら検出（0.1ptから緩和）
        self.min_overflow_chars = 2  # 最低2文字以上のはみ出しが必要
        ...
        self.special_patterns = {
            'closing_bracket': r'^[\)\]\}]+$',  # 閉じ括弧のみ
            'quote_bracket': r'^["\)]+$',  # 引用符と閉じ括弧
            'single_char': r'^.$'  # 単一文字
        }
...
class VisualHybridDetectorV7:
    ...
    def is_likely_false_positive(self, overflow_text: str, overflow_amount: float) -> bool:
        ...
        for pattern_name, pattern in self.config.special_patterns.items():
            if re.match(pattern, overflow_text):
                # 閉じ括弧や引用符のみの場合は、はみ出し量が大きい場合のみ検出
                if pattern_name in ['closing_bracket', 'quote_bracket']:
    ...
    def process_pdf(self, pdf_path: Path) -> List[Dict]:
        ...
        print("視覚的判断優先ハイブリッド検出 v7（精度向上版）")
    ...
    def save_results(self, results: List[Dict], pdf_path: Path, output_path: Path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(f"# {pdf_path.name} 視覚的判断優先ハイブリッド検出結果 v7\n\n")
            f.write("**矩形基準検出を第1優先、座標検出を補助的に使用（精度向上版）**\n\n")
            ...
            f.write(f"- フィルタリング除外: {self.stats['filtered_overflows']}件\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='視覚的判断優先ハイブリッド検出器 v7（精度向上版）'
    )
    ...
    if args.output:
        ...
        output_path = Path(f"testresult_{pdf_path.stem}_visual_v7.md")
    ...
    print(f"フィルタリング除外: {detector.stats['filtered_overflows']}件")
