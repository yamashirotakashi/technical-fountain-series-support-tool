...
def test_pdf_url(url: str) -> Tuple[bool, str]:
    """
...
def main():
    ...
    # テスト用URLを入力
    print("テストするPDF URLを入力してください:")
    print("例1（正常）: http://trial.nextpublishing.jp/upload_46tate/do_download_pdf?n=DWszyxVNUi%2BRBto4Xa8%2FyjA%2BjYdXFlsTir1jImPejtSu%2BIZRwXOD4y5BeT33OTTPZV%2BigZJPiYsnQ603POfqYw%3D%3D")
    print("例2（エラー）: http://trial.nextpublishing.jp/upload_46tate/do_download_pdf?n=QEAhIi0OjyxbOZL1RKha60l4xJF7jv02n42nfHrq6bfYLk%2FN1MhhUp1IKraR3uS2DgTm%2FMQGcEQiAvVpnitNYw%3D%3D")
    print()
    
    while True:
        url = input("URL (qで終了): ").strip()
        if url.lower() == 'q':
            break
        ...
        # URLをテスト
        success, message = test_pdf_url(url)
