...
%%EOF"""
    ...
    # n9999-bottest チャネルにテスト投稿
    target_channel = "n9999-bottest"
    ...
    result = slack.post_pdf_to_channel(
        ...
        message_template="🧪 TechZip Bot のテスト投稿です\n\n生成日時: {timestamp}\nリポジトリ: {repo_name}\n\n✅ PDF投稿機能が正常に動作しています"
    )
    ...
    if result['success']:
        ...
        print("\n📝 次のステップ:")
        print("1. Slackで投稿を確認")
        print("2. 問題なければ、他のチャネルでも同様に動作します")
    else:
        print(f"❌ 投稿失敗: {result['error']}")
        if 'action_required' in result:
            print(f"\n⚠️  必要なアクション: {result['action_required']}")
        if 'instruction' in result:
            print(result['instruction'])
            print("\n💡 ヒント:")
            print("1. Slackアプリで #n9999-bottest チャネルを開く")
